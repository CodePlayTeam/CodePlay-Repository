...
from extend import qiniu_store
...
def sendMail():
    ...
    if not isEmail:
        ...
        msg = Message(subject="Nexus-Website平台注册动态码", recipients=[email])
        msg.html = "<b><img src='http://qiniuyun.donghao.club/nyist.png' " \
                   "style='width:260px'></b><br><b>Nexus-Website平台注册动态码:" \
                   "<font color='red'>" + random_sample + "</font><b>"
        send_mail.send(msg)
        return jsonify({"msg": "", 'status': "200"})
    else:
        return jsonify({"msg": "检测到该邮箱已经被注册，一个邮箱只允许注册一个Nexus平台账号", 'status': "500"})
...
def resetPassword():
    ...
    if user is None:
        ...
        if user.username != name:
            ...
            db.session.commit()
            msg = Message(subject="Nexus-Website平台找回密码", recipients=[email])
            msg.html = "<b><img src='http://127.0.0.1:5000/static/images/nyist.png" \
                       "style='width:260px'></b><br><b>Nexus-Website平台重置你的密码为:" \
